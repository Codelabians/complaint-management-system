...
const ConfirmDialog = ({
  ...
  cancelText = "Cancel",
  isLoading = false,
  variant = "danger", // can be "danger", "warning", "info"
}) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-5 border-b border-gray-200">
          <h3 className={`text-xl font-semibold ${style.title}`}>{title}</h3>
        </div>
        ...
        <div className="px-6 py-4 bg-gray-50 flex justify-end gap-3">
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={isLoading}
            className={`px-5 py-2.5 rounded-lg bg-greenDarkest text-white font-medium transition-colors flex items-center gap-2 disabled:opacity-50 ${style.button}`}
          >
            {isLoading ? (
              <>
